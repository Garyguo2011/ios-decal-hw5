...
class PlayerViewController: UIViewController {
    ...
    func previousTrackTapped(sender: UIButton) {
        ...
        let over3Seconds = (Int(player.currentTime().value) / Int(player.currentTime().timescale)) > 3
        if (over3Seconds || currentIndex <= 0) {
